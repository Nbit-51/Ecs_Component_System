 ...
 *
 * Tradeoffs vs Sparse-Set:
 *   ✅ Perfect cache locality when iterating a single archetype
 *   ✅ Great for worlds where entities rarely change components
 *   ❌ Moving entities between archetypes on add/remove is expensive
 *   ❌ Query matching across many archetypes adds overhead
 *
 * Compile:
